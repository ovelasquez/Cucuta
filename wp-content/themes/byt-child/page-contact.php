...
<?php
	...
	<aside class="right-sidebar lower  one-fourth">
		<!--contact form-->
		<div class="widget">
			<h4><?php esc_html_e('Envíanos un mensaje', 'bookyourtravel'); ?></h4>
			<?php 
			if ($form_submitted) { ?>
			<p>
			<?php
			...
			<form action="<?php echo esc_url(BookYourTravel_Theme_Utils::get_current_page_url()); ?>" id="contact-form" method="post">
				<fieldset class="row">
					...
					<div class="f-item captcha full-width">
						<label><?php echo sprintf(esc_html__('¿Cuánto es %d + %d?', 'bookyourtravel'), $c_val_1, $c_val_2) ?>?</label>
					...
					<div class="f-item full-width"><input type="submit" value="<?php esc_attr_e('Enviar', 'bookyourtravel'); ?>" id="contact_submit" name="contact_submit" class="gradient-button" /></div>
				</fieldset>
			</form>
			<?php } ?>
		</div>
		...
		<!--contact info-->
		<div class="widget">
			<h4><?php esc_html_e('O póngase en contacto con nosotros directamente', 'bookyourtravel'); ?></h4>
