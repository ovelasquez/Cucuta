...
add_action( 'widgets_init', 'bookyourtravel_accommodation_lists_widgets' );

// Register widget.
function bookyourtravel_accommodation_lists_widgets() {
	global $bookyourtravel_theme_globals;
	if ($bookyourtravel_theme_globals->enable_accommodations()) {
		register_widget( 'bookyourtravel_accommodation_list_widget' );
	}
}

// Widget class.
class bookyourtravel_accommodation_list_widget extends WP_Widget {
	...
	function __construct() {
		...
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'bookyourtravel_accommodation_list_widget', 'description' => esc_html__('BookYourTravel: Accommodation List', 'bookyourtravel') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 260, 'height' => 400, 'id_base' => 'bookyourtravel_accommodation_list_widget' );

		/* Create the widget. */
		parent::__construct( 'bookyourtravel_accommodation_list_widget', esc_html__('BookYourTravel: Accommodation List', 'bookyourtravel'), $widget_ops, $control_ops );
	}
...
/*-----------------------------------------------------------------------------------*/
	...
	function widget( $args, $instance ) {
		...
		global $sc_theme_globals, $bookyourtravel_accommodation_helper;
		...
		$title = apply_filters('widget_title', isset($instance['title']) ? $instance['title'] : esc_html__('Explore our latest accommodations', 'bookyourtravel') );
		...
		$accommodation_results = $bookyourtravel_accommodation_helper->list_accommodations(1, $number_of_posts, $sort_by, $order, 0, $accommodation_type_ids, $accommodation_tag_ids, array(), $show_featured_only);
...
/*-----------------------------------------------------------------------------------*/
	...
	function form( $instance ) {
		...
		$defaults = array(
			'title' => esc_html__('Explore our latest accommodations', 'bookyourtravel'),
			'number_of_posts' => '4',
			'sort_by' => 'title',
			'sort_descending' => '1',
			'display_mode' => 'card',
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'number_of_posts' ) ); ?>"><?php esc_html_e('How many accommodations do you want to display?', 'bookyourtravel') ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'sort_by' ) ); ?>"><?php esc_html_e('What do you want to sort the accommodations by?', 'bookyourtravel') ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'sort_descending' ) ); ?>"><?php esc_html_e('Sort accommodations in descending order?', 'bookyourtravel') ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_featured_only') ); ?>"><?php esc_html_e('Show only featured accommodations?', 'bookyourtravel') ?></label>
		...
		<p>
			<label><?php esc_html_e('Accommodation type (leave blank to ignore)', 'bookyourtravel') ?></label>
			<div>
		...
		<p>
			<label><?php esc_html_e('Accommodation tag (leave blank to ignore)', 'bookyourtravel') ?></label>
			<div>
		...
		<p class="cards" <?php echo ( $instance['display_mode'] != 'card' ? 'style="display:none"' : '' ); ?>>
			<label for="<?php echo esc_attr ( $this->get_field_id( 'posts_per_row' ) ); ?>"><?php esc_html_e('How many accommodations do you want to display per row?', 'bookyourtravel') ?></label>
